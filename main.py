...
print(Fore.GREEN + "[x] Check Your Bin Info ")
print(Fore.RED + "--------------------------------------")
print(Fore.BLUE + "Bin     | " + cfg["full_bin"])
print(Fore.BLUE + "Locale  | " + cfg["locale"])
print(Fore.BLUE + "ZipCode | " + cfg["postal_code"])
print(Fore.RED + "--------------------------------------")
...
def main(ms_creds : str , full_card : str):
    ...
    if card.startswith("4"):
        ...
        sprint("[-] Unsupported card! | " + fullBin,"y")
        return
    headers = {
    ...
    try:
        ...
        sprint("[-] Invalid microsoft acc! | " + ms_creds,"c")
...
"t": response.split('id="t" value="')[1].split('"')[0]} 
        ...
        sprint("[-] MS acc cant be used | " + ms_creds,"r")
        return
    heads = {
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
    'Accept-Language': 'en-US,en;q=0.9',
    ...
    while True:
        ...
        "returnUrl": "https://www.xbox.com/en-US/?lc=1033"
            ...
            return
    if not make_acc.ok:
        sprint("[-] Failed to create Xbox Profile | " + ms_creds)
    ...
    try:
        ...
        sprint("[-] Failed to get XBL Auth | " + ms_creds,"y")
        remove_content("accs.txt",ms_creds)
        return
    ...
    while True:
        try:
            ...
            sprint(f"[-] Error in MS-API | " + ms_creds,"c")
            sprint(card_id.text)
            return
        except Exception as e:
            sprint(e,"r")
    ...
    if not vcc_req.ok:
        if "InvalidRequestData" in vcc_req.text:
            vcc_list.insert(0,full_card)
        sprint("[-] Failed to add card! | "  +  ms_creds,"r")
    ...
    if not prof_response.ok:
        sprint('[-] Failed to set billing address! | ' + ms_creds,"c")
    ...
    json=json_data,
            ...
            return
    if not response.ok:
        sprint(f"[-] Failed to buy plan! | " + ms_creds,"y")
        
        return
    try:
        gand = response.json()["cart"]["id"]
        
        sprint(f"[+] Purchased Gamepass! | " + ms_creds,"c")
        vcc_list.insert(0,full_card)
...
def getValidCard():
    ...
    return full_card
colorama.init()
thread_count = int(input(Fore.GREEN+"[x] Type num of threads | "))
...
sprint("[x] Materials Finished  | Reason : Accounts ","y")
sprint("[x] Exiting             | Reason : Materials Finished ","y")
te.sleep(3)
exit(0)